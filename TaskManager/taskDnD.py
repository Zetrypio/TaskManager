...
if __name__=='__main__':
    # Code de test pout tester.
    from task import *
    import datetime
    fen = Tk()
    t = Task("Nom", debut = datetime.datetime(2020, 3, 24), duree = datetime.timedelta(hours = 3), rep = 0, nbrep = 0, desc = "ceci est un test", color = "#FF8888")
    def dnd1(e=None):
        #pos = (fen.winfo_pointerx(), fen.winfo_pointery())
        pos = (fen.winfo_rootx(), fen.winfo_rooty())
        tdnd = TaskInDnd(pos, fen, t, command = print)
    fen.bind("<ButtonPress-1>", dnd1)
    fen.mainloop()
